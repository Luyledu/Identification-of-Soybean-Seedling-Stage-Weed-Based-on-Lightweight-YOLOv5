...
class ConfusionMatrix:
    ...
    names = ['soybean', 'machixian', 'matang', 'tiexiancai', 'ciercai', 'li', 'dawanhua']

    new_order = [0, 1, 6, 2, 4, 3, 5]

    def plot(self, normalize=False, save_dir='', names=(), order = new_order):
        try:
            ...
            if order is not None:
                assert len(order) == nc, f"Order length {len(order)} does not match number of classes {nc}"
    ...
    # def plot(self, normalize=False, save_dir='', names=(),order=None):
    #     try:
    #         import seaborn as sn
    #
